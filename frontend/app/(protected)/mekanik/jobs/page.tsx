...
const statusConfig: Record<string, { label: string; className: string }> = {
  PENDING:       { label: "Pending",       className: "bg-amber-100 dark:bg-amber-500/10 text-amber-600 border-amber-200 dark:border-amber-500/20" },
  ...
  WAITING_PARTS: { label: "Tunggu Parts",  className: "bg-orange-100 dark:bg-orange-500/10 text-orange-600 border-orange-200 dark:border-orange-500/20" },
  QUALITY_CHECK: { label: "Cek Kualitas",  className: "bg-purple-100 dark:bg-purple-500/10 text-purple-600 border-purple-200 dark:border-purple-500/20" },
  ...
  INVOICED:      { label: "Ditagihkan",    className: "bg-teal-100 dark:bg-teal-500/10 text-teal-600 border-teal-200 dark:border-teal-500/20" },
...
const priorityConfig: Record<string, { label: string; className: string }> = {
  ...
  HIGH:   { label: "High",   className: "bg-orange-50 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400" },
  URGENT: { label: "Urgent", className: "bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400" },
}
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-9 bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/5 focus:border-primary/50 transition-all rounded-xl shadow-sm dark:shadow-none"
        />
      </div>
      ...
      <div className="overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as FilterStatus)} className="w-fit">
          <TabsList className="h-10 inline-flex items-center gap-2 bg-white/60 dark:bg-zinc-900/50 p-1 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
            <TabsTrigger value="all" className="rounded-lg text-[10px] font-bold uppercase tracking-wider px-4 data-[state=active]:bg-primary data-[state=active]:text-white dark:data-[state=active]:text-black transition-all">
      ...
      <div className="space-y-3">
        {isLoading ? (
          <Card className="bg-white/50 dark:bg-zinc-900/50 border-slate-200 dark:border-white/5">
            <CardContent className="py-12 text-center">
              <Loader2 className="h-8 w-8 animate-spin text-primary mx-auto mb-3" />
              <p className="text-slate-500 dark:text-zinc-500 font-bold uppercase tracking-widest text-xs">Memuat Tugas...</p>
            </CardContent>
          </Card>
        ) : filteredJobs.length === 0 ? (
          <Card className="bg-white/50 dark:bg-zinc-900/50 border-slate-200 dark:border-white/5 border-dashed">
            <CardContent className="py-12 text-center">
              <ClipboardCheck className="h-12 w-12 text-slate-300 dark:text-zinc-700 mx-auto mb-4" />
              <p className="text-slate-500 dark:text-zinc-500 font-bold uppercase tracking-widest text-xs">Tidak ada SPK ditemukan</p>
            </CardContent>
          </Card>
        ) : (
            ...
            return (
              <Sheet key={job.id}>
                <SheetTrigger asChild>
                  <Card className="cursor-pointer bg-white/80 dark:bg-zinc-900/50 border-slate-200 dark:border-white/5 hover:border-primary/30 dark:hover:border-primary/30 hover:bg-white dark:hover:bg-zinc-900/80 transition-all duration-300 relative overflow-hidden group rounded-2xl shadow-sm hover:shadow-md dark:shadow-none">
                    ...
                    <CardContent className="pt-5 pb-5">
                      ...
                      {/* Customer Info */}
                      <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-zinc-100 uppercase tracking-tight mb-2">
                        <User className="h-4 w-4 text-slate-400 dark:text-zinc-500 group-hover:text-primary transition-colors" />
                        <span>{customerName}</span>
                      </div>

                      {/* Vehicle Info */}
                      <div className="flex items-center gap-2 text-[11px] font-semibold text-slate-500 dark:text-zinc-500 uppercase tracking-widest mb-4">
                        <Car className="h-3.5 w-3.5 text-slate-400 dark:text-zinc-600" />
                        <span>
                      ...
                      {/* Service */}
                      <p className="text-sm font-black text-slate-700 dark:text-zinc-200 uppercase italic mb-1 tracking-tight">{serviceType}</p>
                      <p className="text-xs text-slate-500 dark:text-zinc-500 line-clamp-1 mb-4 font-medium">{complaint}</p>
                ...
                {/* Detail Sheet */}
                <SheetContent side="bottom" className="h-[92vh] bg-slate-50 dark:bg-zinc-950 border-x border-t border-slate-200 dark:border-white/10 rounded-t-[40px] p-0 overflow-hidden max-w-md mx-auto inset-x-0 outline-none">
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto px-6 pb-28 pt-4 space-y-7 custom-scrollbar">
                      <SheetHeader className="text-left space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="outline" className={cn("text-[11px] font-black uppercase italic border-0 px-4 h-8 flex items-center shadow-[inset_0_1px_10px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_1px_10px_rgba(0,0,0,0.5)]", sConfig.className)}>
                        ...
                        <div className="flex items-center gap-4 text-[10px] font-mono text-slate-500 dark:text-zinc-500 uppercase tracking-widest border-y border-slate-200 dark:border-white/5 py-3">
                          ...
                          <span className="h-3 w-px bg-slate-200 dark:bg-white/10 ml-auto" />
                      ...
                      <div className="grid gap-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="bg-white dark:bg-zinc-900/50 rounded-2xl border border-slate-200 dark:border-white/5 p-5 relative overflow-hidden shadow-sm dark:shadow-none">
                            <div className="flex items-center justify-between">
                              <div className="space-y-1">
                                <p className="text-xl font-black text-slate-900 dark:text-zinc-100 uppercase italic tracking-tight">{customerName}</p>
                                <p className="text-xs font-mono text-slate-500 dark:text-zinc-400 flex items-center gap-2">
                                  <Phone className="h-3 w-3" /> {phone}
                                </p>
                              </div>
                              <Button size="icon" variant="outline" className="rounded-xl bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-white/10 hover:border-primary/50 text-primary transition-all active:scale-90">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="bg-white dark:bg-zinc-900/50 rounded-2xl border border-slate-200 dark:border-white/5 p-3 grid grid-cols-3 gap-2 shadow-sm dark:shadow-none">
                            <div className="bg-slate-50 dark:bg-black/40 rounded-xl p-3 border border-slate-100 dark:border-white/5">
                              <p className="text-[8px] text-slate-500 dark:text-zinc-500 font-bold uppercase mb-0.5">Merk/Model</p>
                              <p className="text-[11px] font-black text-slate-800 dark:text-zinc-200 uppercase leading-tight">{brand} {model}</p>
                            </div>
                            <div className="bg-slate-50 dark:bg-black/40 rounded-xl p-3 border border-slate-100 dark:border-white/5">
                            ...
                            <div className="bg-slate-50 dark:bg-black/40 rounded-xl p-3 border border-slate-100 dark:border-white/5">
                              ...
                              <p className="text-[11px] font-black text-slate-800 dark:text-zinc-200 leading-tight">{year || "-"}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm dark:shadow-none">
                            <div className="bg-primary/10 px-5 py-3 border-b border-primary/10 dark:border-white/5 flex items-center justify-between">
                              <span className="text-xs font-black text-primary uppercase italic">{serviceType}</span>
                            </div>
                            <div className="p-5">
                              <div className="bg-slate-50 dark:bg-black/30 rounded-2xl p-4 border border-slate-200 dark:border-white/5 border-dashed">
                    ...
                    <div className="bg-white dark:bg-zinc-950 border-t border-slate-200 dark:border-white/10 p-6 flex items-center gap-4 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] dark:shadow-none">
                      <SheetTrigger asChild>
                        <Button variant="ghost" className="h-14 w-14 rounded-2xl border border-slate-200 dark:border-white/5 text-slate-500 dark:text-zinc-500 hover:bg-slate-100 dark:hover:bg-zinc-900 transition-all active:scale-95">
                          <X className="h-5 w-5" />
                        </Button>
                      </SheetTrigger>
                      <Link href={`/mekanik/jobs/${job.id}`} className="flex-1">
                        <Button className="w-full bg-primary hover:bg-primary/90 text-white dark:text-black font-black uppercase text-sm tracking-[0.15em] h-14 shadow-[0_4px_25px_rgba(var(--primary),0.3)] rounded-2xl border-b-2 border-black/10 dark:border-black/20 group relative overflow-hidden">
