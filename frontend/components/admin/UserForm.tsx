...
const userSchema = z.object({
  ...
  role: z.enum(["ADMIN", "GUDANG", "MEKANIK", "PIMPINAN"]),
...
export function UserForm({ initialData, isEditing = false, onClose }: UserFormProps) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-zinc-400">Peran Akses</FormLabel>
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger className="bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/10 rounded-xl h-11">
                      <SelectValue placeholder="Pilih peran anggota tim" />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
            ...
            <FormItem className="flex flex-row items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 p-4 bg-white dark:bg-zinc-900">
              <div className="space-y-0.5">
                ...
                <FormDescription className="text-xs text-slate-500 dark:text-zinc-500">
                  Anggota tim tidak aktif tidak dapat masuk ke dalam sistem.
                </FormDescription>
              </div>
              <FormControl>
                <Switch
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          >
            ...
            {isEditing ? "Simpan Perubahan" : "Tambah Anggota Tim"}
          </Button>
        </div>
      </form>
    </Form>
